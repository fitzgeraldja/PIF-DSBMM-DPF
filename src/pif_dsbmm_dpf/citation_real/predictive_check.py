...
def evaluate_multi_view_influence(
    items, y, y_p, a, z, w, alpha, gamma, inf, theta=None, kappa=None
):
    users = np.arange(y.shape[0])
    inf_rate = (inf * a).dot(y_p)
...
def main():
    ...
    for e_idx in range(num_exps):
        ...
        for k_idx, K in enumerate(Ks):
            ...
            a_score[e_idx][k_idx] = evaluate_random_subset(
                past_masked_items, Y_past, Theta_hat, W_hat, metric="auc"
            )
            x_score[e_idx][k_idx] = evaluate_random_subset(
                past_masked_items, Y_past, Theta_hat, W_hat, metric="logll"
            )

            smoothness = 100.0
            Theta_random = smoothness * np.random.gamma(
                smoothness, 1.0 / smoothness, size=Theta_hat.shape
            )
            W_random = smoothness * np.random.gamma(
                smoothness, 1.0 / smoothness, size=W_hat.shape
            )

            x_random[e_idx][k_idx] = evaluate_random_subset(
                past_masked_items, Y_past, Theta_random, W_random, metric="logll"
            )
