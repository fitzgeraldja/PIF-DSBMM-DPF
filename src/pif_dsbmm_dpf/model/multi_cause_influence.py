...
class CausalInfluenceModel:
    ...
    def _compute_terms_and_normalizers(
        ...
        if not use_old_subs:
            ...
            Q = Z_trans.shape[0]  # need to take this as could be passing Rho_hat
            # with dims (N,T,Q+K) instead of just (N,T,Q)

            Z[:, :, :Q] = np.einsum("qr,itq->itr", Z_trans, Z[:, :, :Q])
    ...
    def _update_gamma(self, Y, Z):
        norm_obs = [
            np.divide(
                Y_t.toarray(), norm_t, where=norm_t > 0, out=np.zeros(norm_t.shape)
            )
