...
def main():
    ...
    # make adj matrix
    adj_matrix, uid_map, _ = ut.make_matrix(
        friend_data, selected_users, restrict_items=True
    )
    ...
    most_popular = {t[0] for t in activity_counts.most_common(6000)}

    ## get all timestamps that correspond to those activities
    valid_timestamps = [
        activity[1]
        for activity_list in activity_dict.values()
        for activity in activity_list
        if activity[0] in most_popular
    ]
    ...
    past_obs = {
        u: [a[0] for a in act if a[1] <= threshold and a[0] in most_popular]
        for u, act in activity_dict.items()
    }
    valid_acts = {a for u, activities in past_obs.items() for a in activities}
    ...
    present_obs = {
        u: [
            a[0]
            for a in act
            if a[1] > threshold and a[1] <= upper_cutoff and a[0] in valid_acts
        ]
        for u, act in activity_dict.items()
    }
    ...
    heldout_obs = {
        u: [
            a[0]
            for a in act
            if a[1] > upper_cutoff and a[1] <= final_cutoff and a[0] in valid_acts
        ]
        for u, act in activity_dict.items()
    }
