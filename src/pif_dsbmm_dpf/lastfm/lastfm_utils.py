...
def extract_friends(datapath=datadir):
    ...
    for f_idx in file_idx:
        with open(os.path.join(datapath, "egodata_" + str(f_idx) + ".tsv")) as f:
            lines = f.readlines()
...
def get_activities(userid_list, datapath=datadir):
    ...
    for i in range(num_files):
        ...
        with open(
            os.path.join(datapath, "egodata_" + str(i * increments) + ".tsv")
        ) as f:
            lines = f.readlines()
        ...
        for u in users_in_range:
            ...
            if line != "None":
                acts = line.split(" ")
                activity_data[u] = [
                    tuple(map(lambda x: int(x), a.split(":"))) for a in acts
                ]
